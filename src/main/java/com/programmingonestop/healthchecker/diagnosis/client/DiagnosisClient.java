package com.programmingonestop.healthchecker.diagnosis.client;
...
import com.programmingonestop.healthchecker.model.AccessToken;
...
public class DiagnosisClient {
	...
	/// <param name="authServiceUrl">priaid login url
	/// (https://authservice.priaid.ch/login)</param>
	/// <param name="language">language</param>
	/// <param name="healthServiceUrl">priaid healthservice
	/// url(https://healthservice.priaid.ch)</param>
	public DiagnosisClient(String userName, String password, String authServiceUrl, String language,
			String healthServiceUrl) throws Exception {
...
//	        computedHashString = encoder.encode(result); 
	...
	private void HandleRequiredArguments(String username, String password, String authServiceUrl, String language,
	...
	/// <returns>Returns calculated list of potential issues for selected
	/// parameters</returns>
	public List<HealthDiagnosis> loadDiagnosis(List<Integer> selectedSymptoms, Gender gender, int yearOfBirth)
			throws Exception {
		...
		String action = "diagnosis?symptoms=" + serializedSymptoms + "&gender=" + gender.toString() + "&year_of_birth="
				+ yearOfBirth;
	...
	/// <param name="gender">Selected person gender (Male, Female)</param>
	/// <param name="yearOfBirth">Selected person year of born</param>
	/// <returns>Returns calculated list of specialisations for selected
	/// parameters</returns>
	public List<DiagnosedSpecialisation> loadSpecialisations(List<Integer> selectedSymptoms, Gender gender,
			int yearOfBirth) throws Exception {
		if (selectedSymptoms == null || selectedSymptoms.size() == 0)
			throw new IllegalArgumentException("selectedSymptoms  Can not be null or empty");

		String serializedSymptoms = new ObjectMapper().writeValueAsString(selectedSymptoms);
		String action = "diagnosis/specialisations?symptoms=" + serializedSymptoms + "&gender=" + gender.toString()
				+ "&year_of_birth=" + yearOfBirth;
		return this.<List<DiagnosedSpecialisation>>loadFromWebService(action,
				new TypeReference<List<DiagnosedSpecialisation>>() {
				});
	}
	...
	/// <returns>Returns list of human body sublocations for selected human body
	/// location</returns>
	...
	/// <param name="selectedSelectorStatus">Selector status (Man, Woman, Boy,
	/// Girl)</param>
	/// <returns>Returns list of all symptoms for selected human body
	/// location</returns>
	public List<HealthSymptomSelector> loadSublocationSymptoms(int locationId, SelectorStatus selectedSelectorStatus)
		...
		return this.<List<HealthSymptomSelector>>loadFromWebService(action,
				new TypeReference<List<HealthSymptomSelector>>() {
				});
	}
	...
	/// <param name="gender">Selected person gender (Male, Female)</param>
	/// <param name="yearOfBirth">Selected person year of born</param>
	/// <returns>Returns list of proposed symptoms for selected symptoms
	/// combination</returns>
	public List<HealthItem> loadProposedSymptoms(List<Integer> selectedSymptoms, Gender gender, Integer yearOfBirth)
			throws Exception {
		if (selectedSymptoms == null || selectedSymptoms.size() == 0)
			throw new IllegalArgumentException("selectedSymptoms  Can not be null or empty");

		String serializedSymptoms = new ObjectMapper().writeValueAsString(selectedSymptoms);
		String action = "symptoms/proposed?symptoms=" + serializedSymptoms + "&gender=" + gender.toString()
				+ "&year_of_birth=" + yearOfBirth;
